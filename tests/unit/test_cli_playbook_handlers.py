...
# -- cmd_check_fresh ---------------------------------------------------------
def _report(is_fresh=True, drift=None, instance_label="box"):
    return SimpleNamespace(is_fresh=is_fresh, drift=drift or [], instance_label=instance_label)


def test_check_fresh_unstamped_returns_1(capfd):
    conn = SimpleNamespace(close=lambda: None)
    with (
        patch.object(pb, "_catalog_conn", return_value=(conn, "cat.db")),
        patch("fsr_playbooks._catalog_meta.get_all", return_value={}),
    ):
        assert pb.cmd_check_fresh(_args()) == 1
    assert "no provenance stamp" in capfd.readouterr().err


def test_check_fresh_up_to_date_returns_0(capfd):
    conn = SimpleNamespace(close=lambda: None)
    with (
        patch.object(pb, "_catalog_conn", return_value=(conn, "cat.db")),
        patch("fsr_playbooks._catalog_meta.get_all", return_value={"base_url_hash": "h", "fsr_version": "8.0.0"}),
        patch.object(pb, "_make_client"),
        patch("pyfsr.playbook_freshness.probe_live", return_value={"version": "8.0.0"}),
        patch("pyfsr.playbook_freshness.compare", return_value=_report(is_fresh=True)),
    ):
        assert pb.cmd_check_fresh(_args()) == 0
    assert "up to date" in capfd.readouterr().err


def test_check_fresh_drift_returns_2(capfd):
    conn = SimpleNamespace(close=lambda: None)
    with (
        patch.object(pb, "_catalog_conn", return_value=(conn, "cat.db")),
        patch("fsr_playbooks._catalog_meta.get_all", return_value={"base_url_hash": "h", "fsr_version": "7.6.5"}),
        patch.object(pb, "_make_client"),
        patch("pyfsr.playbook_freshness.probe_live", return_value={"version": "8.0.0"}),
        patch("pyfsr.playbook_freshness.compare", return_value=_report(is_fresh=False, drift=["version changed"])),
    ):
        assert pb.cmd_check_fresh(_args()) == 2
    assert "drift detected" in capfd.readouterr().err
