...
### Added
- **`MCPServerStatus` model + `AIApi.mcp_status()`** -- the runtime health
  probe for registered MCP servers (`GET /api/ai/mcp/status`). Returns one
  entry per server with a green/red liveness verdict (`valid` / `error`),
  the same probe the agent UI uses. Complements `list_mcp_servers()`
  (id+name only) and `mcp_configs()` (full records). Live-verified on 8.0.0.
- **FortiAI agent catalog** -- 11 example playbooks (one per on-appliance
  FortiAI agent) packaged as a single importable collection. Each playbook
  triggers a named agent via `POST /api/ai/agents/<name>/trigger`, waits for
  completion, fetches the result, and writes it back as a comment. Covers
  both result families: verdict agents (`answer`/`confidence`/`evidence`)
  and structured agents (`data`).
- **`probe_fortiai_agents.py`** -- discovery script that documents each
  agent's `inputformat` schema and probes the trigger endpoint with an empty
  payload to reveal undocumented required fields.
- **`fortiai-agent-ioc-enrichment.yaml`** -- standalone example playbook
  calling a named FortiAI agent (not the raw chat-completions proxy).
- `fortinet-fortiai-proxy` added to the library fixture connectors with its
  `agent_chat_completions` operation.

### Fixed
- **Em-dash / en-dash purge** -- replaced all U+2013/U+2014 characters in
  source, docstrings, and example YAMLs with ASCII equivalents (`-` / `--`).
  These characters break FortiSOAR's latin-1 connector HTTP encoding at
  runtime (`'latin-1' codec can't encode character`).
